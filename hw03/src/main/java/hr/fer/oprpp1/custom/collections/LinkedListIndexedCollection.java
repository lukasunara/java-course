...
/**
 * Instances of this class implement {@link List} and represent a linked list-backed collection of Objects.
...
public class LinkedListIndexedCollection<E> implements List<E> {
	 ...
	 * @author lukasunara
	 * 
	 * @param <T> type of Object which this ListNode can storage
	 */
	...
	public void add(E value) {
		...
		ListNode<E> newNode = new ListNode<>(value, last, null);
	 ...
	 * way does the ElementsGetter iterate over every element of the {@link LinkedListIndexedCollection}.
	 * 
	 * @author lukasunara
	 *
	...
	private static class ListElementsGetter<T> implements ElementsGetter<T> {
		...
		/** {@link LinkedListIndexedCollection} we want to iterate over **/
		private LinkedListIndexedCollection<T> linkedListCollection;

		/** Used to see if collection was altered (altered when != linkedListCollection.modificationCount) **/
		private long savedModificationCount;
		
		/** The current node in the {@link LinkedListIndexedCollection} **/
		private LinkedListIndexedCollection.ListNode<T> currentNode;
		
		/**
		 * Creates an instance of {@link ListElementsGetter} and creates a reference on the
		 * given {@link LinkedListIndexedCollection}.
		 * 
		 * @param linkedListCollection is the collection we want to iterate over.
		 */
		public ListElementsGetter(LinkedListIndexedCollection<T> linkedListCollection) {
			super();
			this.linkedListCollection = linkedListCollection;
			this.currentNode = this.linkedListCollection.first;
			this.savedModificationCount = linkedListCollection.modificationCount;
		}
		
		/**
		 * Checks if there exists another element in the {@link LinkedListIndexedCollection}.
		 * 
		 * @throws ConcurrentModificationException if collection has been altered
		 */
		@Override
		public boolean hasNextElement() {
			if(savedModificationCount != linkedListCollection.modificationCount)
				throw new ConcurrentModificationException("LinkedListIndexedCollection has been altered!");
			
			return currentNode != null;
		}

		/**
		 * Checks if there exists another element in the {@link LinkedListIndexedCollection}, returns
		 * the Object and moves past it (iterates over an element in the list).
		 * 
		...
		public T getNextElement() {
			if(savedModificationCount != linkedListCollection.modificationCount)
				throw new ConcurrentModificationException("LinkedListIndexedCollection has been altered!");
			if(!hasNextElement())
				throw new NoSuchElementException("LinkedListIndexedCollection is empty!");
		...
		/**
		 * @throws NoSuchElementException if there are not any elements remaining in the list
		 */
		@Override
	...
	public ElementsGetter<E> createElementsGetter() {
		ElementsGetter<E> eg = new ListElementsGetter<>(this);
